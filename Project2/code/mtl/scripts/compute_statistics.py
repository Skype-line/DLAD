...
if __name__ == '__main__':
    ...
    # add log depth statistics
    depth_log_accum, depth_log_accum2 = 0.0, 0.0
    ...
    for batch in tqdm(dl):
        ...
        # add log depth statistics
        depth_log_accum += torch.log(depth).sum().cpu().item()
        depth_log_accum2 += (torch.log(depth) ** 2).sum().cpu().item()
    ...
    depth_log_mean = depth_log_accum / depth_cnt
    depth_log_mean2 = depth_log_accum2 / depth_cnt
    depth_log_std = math.sqrt(depth_log_mean2 - depth_log_mean ** 2)


    print(f'rgb mean={rgb_mean} std={rgb_std}')
    print(f'depth mean={depth_mean} std={depth_std} min={depth_min} max={depth_max}')
    print(f'depth log mean={depth_log_mean} std={depth_log_std}')
