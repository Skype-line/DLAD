...
class ZeroMeanUnitVarianceLogTensor:
    def __init__(self, modality, mean, stddev):
        self.modality = modality
        self.mean = mean
        self.stddev = stddev
    ...
    def __call__(self, sample):
        ...
        mean = torch.tensor(self.mean, dtype=torch.float32).unsqueeze(-1).unsqueeze(-1)
        stddev = torch.tensor(self.stddev, dtype=torch.float32).unsqueeze(-1).unsqueeze(-1)
        sample[self.modality] = (torch.log(sample[self.modality]) - mean) / stddev
        return sample
...
def get_transforms(
    ...
    if if_log:
        return Compose([
            ...
            ZeroMeanUnitVarianceLogTensor(MOD_DEPTH, depth_meters_mean, depth_meters_stddev) if depth_meters_mean is not None else Identity(),
        ])
    else:
        return Compose([
            CropForPassableSidesPIL(crop_for_passable) if crop_for_passable > 0 else Identity(),
