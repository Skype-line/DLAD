...
class DatasetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features = self.get_data(idx, 'features')
        norms = norm(points, axis=1, keepdims=True)/70-0.5
        if self.config['if_xyz_mlp'] and not self.config['if_bin_loss']:
            features = np.concatenate((points,norms,features),axis=-1)
        ...
        if self.config['if_bin_loss']:
            assigned_pred, assinged_target, xyz, feat, iou = sample_proposals(pred=valid_pred,
                                                            target=target,
                                                            xyz=pooled_xyz,
                                                            feat=pooled_feat,
                                                            config=self.config,
