...
from utils.task4 import RegressionLoss, ClassificationLoss, RegressionIOULoss, RegressionBinLoss, decode_bbox_target
...
class LitModel(pl.LightningModule):
    def __init__(self, config):
        ...
        self.iou_loss = RegressionIOULoss(config['loss'])
        self.bin_reg_loss = RegressionBinLoss(config['loss'])
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config['loss']['if_bin_loss']:
            reg_loss = self.bin_reg_loss(pred_box, assinged_target, iou)
        else:
            reg_loss = self.reg_loss(pred_box, assinged_target, iou)
        cls_loss = self.cls_loss(pred_class, iou)
        loss = reg_loss + cls_loss
        self.log('train_loss', loss, on_step=True, on_epoch=True, prog_bar=True)
        # train using iou loss
        if self.config['loss']['if_iou_loss']:
            # change input box   
            iou_loss = self.iou_loss(pred_box, assinged_target, iou)
            loss = iou_loss + cls_loss + reg_loss
            self.log('train_iou_loss', iou_loss, on_step=True, on_epoch=True, prog_bar=True)
            self.log('train_reg_loss', reg_loss, on_step=True, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.config['loss']['if_iou_loss']:
            ...
            iou_loss = self.iou_loss(pred_box, assinged_target, iou)
            loss = iou_loss + cls_loss + reg_loss
            self.log('valid_iou_loss', iou_loss, on_step=False, on_epoch=True, prog_bar=True)
            self.log('valid_reg_loss', reg_loss, on_step=False, on_epoch=True, prog_bar=True)
            self.log('valid_cls_loss', cls_loss, on_step=False, on_epoch=True, prog_bar=True)
            loss += iou_loss * 2.0
            self.log('valid_loss', loss, on_step=False, on_epoch=True, prog_bar=True)

        # compute outputs
        if self.config['loss']['if_bin_loss']:
            assinged_pred = batch["assigned_pred"]
            anchor_size = pred_box[:, 3:6]
            pred_box = decode_bbox_target(assinged_pred, pred_box, 
                                        self.config['loss']['LOC_SCOPE'],
                                        self.config['loss']['LOC_BIN_SIZE'],
                                        self.config['loss']['NUM_HEAD_BIN'],
                                        anchor_size,
                                        True,
                                        self.config['loss']['LOC_Y_BY_BIN'],
                                        loc_y_scope=self.config['loss']['LOC_Y_SCOPE'], loc_y_bin_size=self.config['loss']['LOC_Y_BIN_SIZE'],
                                         get_ry_fine=True)
    ...
    def configure_optimizers(self):
        if self.config['optimizer']['Adam']:
            ...
            optimizer = optim.SGD(self.parameters(), lr=self.config['optimizer']['lr'], momentum=self.config['optimizer']['momentum'])
